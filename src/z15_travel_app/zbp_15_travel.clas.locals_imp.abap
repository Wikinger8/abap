...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD ValidateCustomer.
    DATA travels TYPE TABLE FOR READ RESULT ZR_15_TravelTP.

    " Read Travels

    READ ENTITIES OF ZR_15_TravelTP IN LOCAL MODE
         ENTITY Travel
         FIELDS ( CustomerId )
  ...
  METHOD DetermineStatus.
    DATA travels TYPE TABLE FOR UPDATE zr_15_travelTp.

    LOOP AT keys INTO DATA(key).
      APPEND VALUE #( %tky = key-%tky Status = 'N' ) TO travels.
    ENDLOOP.

    MODIFY ENTITIES OF ZR_15_TravelTP IN LOCAL MODE ENTITY Travel
           UPDATE
           FIELDS ( Status )
           WITH travels.
  ENDMETHOD.
